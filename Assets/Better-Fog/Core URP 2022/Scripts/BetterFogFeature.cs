	...
	public class BetterFogFeature : ScriptableRendererFeature
	{
		...
		public class BetterFogPass : ScriptableRenderPass
		{
			...
			public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
			{
                ...
                RenderingUtils.ReAllocateIfNeeded(ref m_TemporaryRT, textureDescriptor, FilterMode.Point, TextureWrapMode.Clamp, name: "_TemporaryRT");

				textureDescriptor.depthBufferBits = 0; // 32 bits breaks everytinhg
				textureDescriptor.colorFormat = RenderTextureFormat.RHalf;
				RenderingUtils.ReAllocateIfNeeded(ref m_FogFactorRT, textureDescriptor, FilterMode.Point, TextureWrapMode.Clamp, name: "_FogFactor_RT");

				if (m_CurrentSettings._UseCustomDepthTexture)
                {
					//textureDescriptor.depthBufferBits = 32;
					textureDescriptor.colorFormat = RenderTextureFormat.RFloat;
					RenderingUtils.ReAllocateIfNeeded(ref m_CustomDepthRT, textureDescriptor, FilterMode.Point, TextureWrapMode.Clamp, name: "_CustomDepth");
				}
				...
				if (m_CurrentSettings._UseFogOffsetTexture)
				{
					...
					RenderingUtils.ReAllocateIfNeeded(ref m_FogOffsetRT, textureDescriptor, FilterMode.Point, TextureWrapMode.Clamp, name: "_FogOffset");
				}
			}
			...
			public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
			{
				...
				else
				{
					...
					RenderingUtils.ReAllocateIfNeeded(ref prefilteredRT, prefilteredDesc, rtFilterMode, TextureWrapMode.Clamp);
					...
					for (var level = 0; level < iterations; level++)
					{
						...
						RenderingUtils.ReAllocateIfNeeded(ref _blurBuffer1[level], blurBufferDesc, rtFilterMode, TextureWrapMode.Clamp);
						RenderingUtils.ReAllocateIfNeeded(ref _blurBuffer2[level], blurBufferDesc, rtFilterMode, TextureWrapMode.Clamp);
